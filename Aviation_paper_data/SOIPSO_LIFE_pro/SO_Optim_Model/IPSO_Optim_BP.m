...
%% 导入数据
data = readmatrix('Numerical_experiment.csv');  % BLISK4.csv(叶盘数据)    Numerical_experiment.csv（数值实验）

%% 生成训练集、测试集
temp = randperm(150);   % 叶盘数据为2000   数值实验为200

m = 100;               % 叶盘数据为1500   数值实验为150
P_train = data(temp(1:m),1:2)';    % 叶盘数据为1:4   数值实验为1:2
T_train = data(temp(1:m),3)';      % 5是叶片的应力输出，7是盘的应力输出。  [数值实验]为3
M = size(P_train,2);

P_test = data(temp(m+1:end),1:2)';
T_test = data(temp(m+1:end),3)';   % 5是叶片的应力输出，7是盘的应力输出。
N = size(P_test,2);
...
string = {['RMSE with PSO-BP =' num2str(error2)]};
title(string);
xlim([1,N]);
grid on
...
xlabel('Prediction sample');
ylabel('Prediction result');
string = {['RMSE with BP = ' num2str(error4)]};
title(string);
xlim([1,N]);
grid

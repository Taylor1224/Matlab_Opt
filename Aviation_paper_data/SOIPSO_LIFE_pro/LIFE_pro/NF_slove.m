...
fid = fopen('D_LIFE0.5.txt','wt');      % wt有重新写入的作用
fprintf(fid,'%g\n',Life_x); 
fid = fopen('D_LIFE0.9.txt','wt');
fprintf(fid,'%g\n',Life_y); 
fid = fopen('D_LIFE0.95.txt','wt');
fprintf(fid,'%g\n',Life_z); 
fid = fopen('D_LIFE0.5_optim.txt','wt'); 
fprintf(fid,'%g\n',Life_Optim_x); 
fid = fopen('D_LIFE0.9_optim.txt','wt');
fprintf(fid,'%g\n',Life_Optim_y); 
fid = fopen('D_LIFE0.95_optim.txt','wt');
fprintf(fid,'%g\n',Life_Optim_z); 
...
B1 = load('LIFE0.5.txt');
subplot(1,2,1);
histfit(B1(:,1),100,'lognorm');
xlabel('Blade life');
ylabel('Relative frequency');

D1 = load('D_LIFE0.5.txt');
subplot(1,2,2);
histfit(D1(:,1),100,'lognorm');
xlabel('Disk life');
ylabel('Relative frequency');
...
B2 = load('LIFE0.9.txt');
subplot(1,2,1);
histfit(B2(:,1),100,'lognorm');
xlabel('Blade life');
ylabel('Relative frequency');

D2 = load('D_LIFE0.9.txt');
subplot(1,2,2);
histfit(D2(:,1),100,'lognorm');
xlabel('Disk life');
ylabel('Relative frequency');

  % 置信度0.95下，叶片B1和盘D寿命样本分布图：
B3 = load('LIFE0.95.txt');
subplot(1,2,1);
histfit(B3(:,1),100,'lognorm');
xlabel('Blade life');
ylabel('Relative frequency');

D3 = load('D_LIFE0.95.txt');
subplot(1,2,2);
histfit(D3(:,1),100,'lognorm');
xlabel('Disk life');
ylabel('Relative frequency');
...
data= load('LIFE0.95.txt');data1= load('LIFE0.95_optim.txt');
subplot(1,2,1);
histfit(data,100,'lognorm');
title('优化前的寿命');

subplot(1,2,2);
histfit(data1,100,'lognorm');
title('优化后的寿命')

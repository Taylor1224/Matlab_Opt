...
%% 优化后的应变和应力样本：
% BP神经网络模型优化求得的：
BP_mu = 908.1248;BP_sigma = 17.3731;

% 优化后名义应变/应变范围muX(1)，名义应力/应力范围x_stress：
muX = [BP_mu;161000];
...
for j = 1:nS1
    syms stresstrue
    stresstrue = abs(vpasolve(Kt^2*x_strain(j,1)*x_stress(j,1)-stresstrue^2/x_optim(j,2)-stresstrue*(stresstrue/K_)^(1/n_) == 0,stresstrue)); 
...
for n = 1:nS1
    syms stressrange
    stressrange = abs(vpasolve(Kt^2*x_strain(n,1)*x_stress(n,1)-stressrange^2/x_optim(n,2)-2*stressrange*(stressrange/(2*K_))^(1/n_) == 0,stressrange)); 
...
fid=fopen('D_data1.txt','wt');%写入文件路径
[m,n]=size(a); 
 for i=1:1:m
    for j=1:1:n
       if j==n

function y_pred = RMLS_Fun_Nf_2ci(x_pred1)
    %%%%%%%%%%%% x_pred1是叶片的随机变量；x_pred2是盘的随机变量 ；y_pred是叶盘系统的预测寿命 %%%%%%%%%%%%%
    K_ =1420 ;n_ = 0.05;E = 161000;
    h = 1002;  % 核函数带宽
    N = 100;
    data = readmatrix('BLISK4.csv');
    temp = randperm(N);    % 随机取50组
    X1 = data(temp,1:4)';   % 4个随机变量样本
    Y1 = data(temp,5)';     % 叶片应力样本
    Y2 = data(temp,7)';     % 盘应力样本
    % 样本数量和变量维度
    [n, d] = size(X1);
    % 核函数的定义 (高斯核)
    ...
    A1_1 = [ones(1,d); X1; X1.^2];  
    % 执行加权最小二乘法: 通过正规方程计算系数
    theta1_1 = (A1_1 * W1_1 * A1_1') \ (A1_1 * W1_1 * Y1');
    % 使用拟合的局部多项式来计算预测点的y值
    stress1 = [1, x_pred1', x_pred1'.^2] * theta1_1;
    ...
    % 构造A矩阵，A = [1, x1, x2, x3, x4]
    A1_2 = [ones(1,d); X1; X1.^2];
    % 执行加权最小二乘法: 通过正规方程计算系数
    theta1_2 = (A1_2 * W1_2 * A1_2') \ (A1_2 * W1_2 * Y2');
    % 使用拟合的局部多项式来计算预测点的y值
    stress2 = [1, x_pred1', x_pred1'.^2] * theta1_2;
    ...
    % 构造A矩阵，A = [1, x1, x2, x3, x4]
    A2_1 = [ones(1,d); X2; X2.^2];
    % 执行加权最小二乘法: 通过正规方程计算系数
    theta2_1 = (A2_1 * W2_1 * A2_1') \ (A2_1 * W2_1 * Y3');
    % 使用拟合的局部多项式来计算预测点的y值
    Nf1 = [1, X2_pred', X2_pred'.^2] * theta2_1;
    
    % 第二级计算盘寿命Nf2:
    %%%%%%% MC抽样盘的寿命，为了得到Y4(需要应力应变的原始样本) %%%%%%%
    Y2;%盘应力
    x_strain2 = ones(N,1);
    x_stress2 = Y2';
    for i = 1:N
    ...
    % 构造A矩阵，A = [1, x1, x2, x3, x4]
    A2_2 = [ones(1,d); X3; X3.^2];
    % 执行加权最小二乘法: 通过正规方程计算系数
    theta2_2 = (A2_2 * W2_2 * A2_2') \ (A2_2 * W2_2 * Y4');
    % 使用拟合的局部多项式来计算预测点的y值
    Nf2 = [1, X3_pred', X3_pred'.^2] * theta2_2;

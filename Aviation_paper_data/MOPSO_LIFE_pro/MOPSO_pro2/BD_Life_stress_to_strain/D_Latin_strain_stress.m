...
%% Latin超立方抽样参数：
nS = 1e5;nS1 = 1e4;K_ =1420 ;n_ = 0.04;Kt = 2.32; % 若优化后由应力得出的应变大了，可将n_调小一点！或者把K_再调大一点！
...
%% 优化后的应变和应力样本：
% RSM模型优化后应力的均值和方差：
RSM_mu = 9.107916875941557e+02; RSM_sigma = 18.0110;

% 名义应力/应力范围muX(1)，名义应变/应变范围x_strain：
muX = [RSM_mu;161000];
...
a = [x(:,3),x(:,1),x(:,2),x_strain,x_optim(:,1),x1(:,:),x2(:,:),x3(:,:)];
fid=fopen('D_data.txt','wt');%写入文件路径
[m,n]=size(a); 
 for i=1:1:m
    for j=1:1:n
       if j==n

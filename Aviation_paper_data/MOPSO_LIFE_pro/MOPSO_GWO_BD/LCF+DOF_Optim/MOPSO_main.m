...
for it = 1:MaxIt
    ...
    for i = 1:nPop
        ...
        % 更新 alpha, beta, delta
        if Dominates(pop(i), alpha_score)
            ...
            alpha_score = pop(i);
            alpha_position = pop(i).Position;
        elseif Dominates(pop(i), beta_score)
            delta_score = beta_score;
            delta_position = beta_position;
            
            beta_score = pop(i);
            beta_position = pop(i).Position;
        elseif Dominates(pop(i), delta_score)
            delta_score = pop(i);
            delta_position = pop(i).Position;
        end
    ...
    for i = 1:nPop
        ...
        A1 = 2 * rand(nVar, 1) - 1;
        C1 = 2 * rand(nVar, 1);
        D_alpha = abs(C1 .* alpha_position' - pop(i).Position');
        X1 = alpha_position' - A1 .* D_alpha;
        
        A2 = 2 * rand(nVar, 1) - 1;
        C2 = 2 * rand(nVar, 1);
        D_beta = abs(C2 .* beta_position' - pop(i).Position');
        X2 = beta_position' - A2 .* D_beta;
        
        A3 = 2 * rand(nVar, 1) - 1;
        C3 = 2 * rand(nVar, 1);
        D_delta = abs(C3 .* delta_position' - pop(i).Position');
        ...
        pop(i).Velocity = w * pop(i).Velocity + ...
                           ...
                           c2 * rand * (alpha_position - pop(i).Position);
        % Position update
        pop(i).Position = new_position' + pop(i).Velocity;
    ...
    pop(i).Best.Position = alpha_position;
    pop(i).Best.Cost = alpha_score.Cost;

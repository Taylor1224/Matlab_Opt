...
%% 导入数据
data = csvread('BLISK4.csv');

%% 生成训练集、测试集
temp = randperm(2000);  % 将1-2000中的整数打乱后赋值给temp.

m = 1500;
P_train = data(temp(1:m),1:4)';
T_train = data(temp(1:m),10)';      % 5是叶片的应力输出，7是盘的应力输出，9是叶片的径向变形输出，10是盘的径向变形输出。
M = size(P_train,2);

P_test = data(temp(m+1:end),1:4)';
T_test = data(temp(m+1:end),10)';   % 5是叶片的应力输出，7是盘的应力输出，9是叶片的径向变形输出，10是盘的径向变形输出。
N = size(P_test,2);
...
% 普通初始化种群：
% for i = 1:sizepop
%     pop(i,:) = rands(1, numsum);   % rand(1,5)表示从0-1之间随机抽5个数；rands(1,5)表示从-1-1之间随机抽5个数。
%     V(i, :) = rands(1, numsum);
...
% net = train(net,Pn_train,Tn_train);  这里不需要
...
net1_new = train(net, Pn_train, T_sim1);   % 叶片的应力
net2_new = train(net, Pn_train, T_sim1);   % 盘的应力
net3_new = train(net, Pn_train, T_sim1);   % 叶片的径向变形
net4_new = train(net, Pn_train, T_sim1);   % 盘的径向变形
...
string = {'Comparison of prediction results between test sets';['RMSE=' num2str(error2)]};
title(string);
xlim([1,N]);
grid
...
xlabel('Prediction sample');
ylabel('Prediction result');
string = {'Comparison of prediction results between test sets';['RMSE=' num2str(error4)]};
title(string);
xlim([1,N]);
grid
...
figure
scatter(T_test, T_sim4, sz, c)
hold on
plot(xlim, ylim, '--k')
xlabel('测试集真实值');
ylabel('测试集预测值');
xlim([min(T_test) max(T_test)])
ylim([min(T_sim4) max(T_sim4)])
title('测试集预测值 vs. 测试集真实值')
